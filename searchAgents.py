...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        return (self.startingPosition, initial_visited)
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                next_state = (next_position, tuple(next_visited))
                cost = 1
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    unvisited_corners = [
        corners[i] for i in range(len(corners)) if not visited_corners[i]
    ]
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    pacman_to_foods = [
        util.manhattanDistance(position, food_pos) for food_pos in food_list
    ]
    max_pacman_to_food = max(pacman_to_foods)
    ...
    if len(food_list) > 1:
        for i in range(len(food_list)):
            for j in range(i + 1, len(food_list)):
                ...
                if dist > max_food_to_food:
                    max_food_to_food = dist

    return max(max_pacman_to_food, max_food_to_food)
